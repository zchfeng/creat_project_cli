 ...
 * @Date: 2022-01-14 16:35:03
 * @LastEditors: chengfengZeng
 * @LastEditTime: 2022-01-17 16:44:53
 */
// 请求 log-symbols 库
...
async function mirrorAction(link) {
  try {
    ...
    // 再写入 config.json 文件
    console.log(cfgPath, jsonConfig);
    await fse.writeJson(cfgPath, jsonConfig);
    // 等待写入后再提示配置成功
    console.log("Set the mirror successful.");
  } catch (err) {
    // 如果出错，提示报错信息
    console.log(chalk.red(`Set the mirror failed. ${err}`));
    process.exit();
  }
}
